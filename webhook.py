...
from core.store import stats, hero_description, locations, enemies, translations, hero_attack_prompt, \
    enemies_attack_prompt, abilities
...
def webhook(session):
    ...
    if action == 'hero_introduction':
    ...
    elif action == 'update_cur_turn_count':
        session["cur_turn_count"] += 1
        print(session["cur_turn_count"], session["turns"][str(session["cur_turn_count"])]["name"])
        ...
        print("enemy_move")
        target = random.choice(list(random.choice(list(session['players_data'].keys()))))
        attack_res = random.choices([1, 2], weights=[3, 1])
        if attack_res == 1:
            damage = roll_dice(4, session["cur_loc_enemies"][session["cur_person_name"]]["attack"])
            enemy_move_prompt = f'{target}, {session["cur_person_name"]} {random.choice(enemies_attack_prompt["usual"])}. Он наносит {damage} урона.'
            session['players_data'][target]["health"] -= damage
        else:
            enemy_move_prompt = f'{target}, {session["cur_person_name"]} {random.choice(enemies_attack_prompt["missed"])}. Он не наносит урона.'
        ...
        if session["cur_loc_enemies"][target]["health"] <= 0:
            ...
            session["turns"] = {key: val for key, val in session["turns"].items() if val != target}
